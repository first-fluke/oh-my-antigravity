...
テストと受入基準は単一スキルの作業にも含まれます。それだけで`/ralph`が必要になるわけではありません。ドメイン間の調整や、明示的に依頼する品質プロセスについては、[スキルとワークフローの選び方](/docs/core-concepts/workflows#choosing-a-skill-or-workflow)を参照してください。

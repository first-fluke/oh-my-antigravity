...
単一ドメインのタスクには特別な構文は不要です。単一スキル、`/work`、`/orchestrate`、`/ultrawork`、`/ralph`の選択には、[スキルとワークフローの選び方](/docs/core-concepts/workflows#choosing-a-skill-or-workflow)を参照してください。
...
1. この依頼はフロントエンド、バックエンド、モバイルにまたがります。ホストエージェントはその範囲に応じて、作業の調整方法を提案できます。
2. キーワード検出フックが有効な場合、英語の「Build a TODO app」は設定済みの`/orchestrate`パターンに一致し、起動につながる場合があります。フックは文字列を照合しており、依頼をマルチドメインと分類しているわけではありません。希望するワークフローは明示的なコマンドで選択してください。

**`/work`を使用（ステップバイステップ）：**

1. **PMエージェントが計画：** ドメイン特定、APIコントラクト定義、優先タスク分解
2. **プランをレビュー：** エージェントはプランを提示し、既存の承認の範囲で続行します。確認するのは、未決定の重要な事項や新たな承認が必要な場合だけです。
...
- PLAN_GATE：プランを文書化し、前提を列挙し、作業範囲の承認を得ます。
- IMPL_GATE：適用対象のチェック（成果物の出力なし）とテストが通り、変更は計画したファイルのみです。ビルドチェックは明示的に依頼された場合のみ実行します。
- SHIP_GATE：すべてのチェックが通ったら、既存の承認を引き継ぎます。公開やデプロイには、その操作への承認が必要です。
...
| コマンド | 種別 | 内容 | 使用場面 |
|---------|------|-------------|-------------|
| `/orchestrate` | 永続 | プランをロードまたは作成し、監視と検証を伴う並列実行を委任 | 自動並列協調に適した独立タスク |
| `/work` | 永続 | 承認済みの範囲で複数ドメインの計画・実装・QAを段階的に調整 | 複数ドメインにまたがり、連携して完成させる必要がある機能 |
...
| `/ralph` | 永続 | 独立したjudgeとループのセーフガードを伴うultraworkの反復実行 | 機械的に検証できる完了基準を満たすまで実行を繰り返すという明示的な依頼 |
...
6. **タスクに合わせて調整方法を選ぶ。** 単一ドメインなら単一スキルから始めます。協調や明示的な品質プロセスが必要な場合は、[選択ガイド](/docs/core-concepts/workflows#choosing-a-skill-or-workflow)を参照してください。
